...
import { SemanticAnalyzer } from '@inline/language';
...
export class RefactoringActions {
    ...
    async renameSymbol(
        ...
        while ((match = regex.exec(text)) !== null) {
            ...
            const matchRange = new vscode.Range(
                matchPosition,
                document.positionAt(match.index + oldName.length)
            );
    ...
    async inlineVariable(
        ...
        while ((match = usageRegex.exec(text)) !== null) {
            ...
            if (matchLine === declLine) continue;
     ...
     */
    private findUsedVariables(code: string, document: vscode.TextDocument, range: vscode.Range): string[] {
     ...
     */
    private findInsertPositionForFunction(document: vscode.TextDocument, range: vscode.Range): vscode.Position {
