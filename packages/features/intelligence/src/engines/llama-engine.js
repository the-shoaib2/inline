...
exports.LlamaInference = void 0;
const fs = __importStar(require("fs"));
const shared_1 = require("@inline/shared");
const gpu_detector_1 = require("../optimization/gpu-detector");
const language_1 = require("@inline/language");
const language_2 = require("@inline/language");
const fim_templates_1 = require("../processing/fim-templates");
...
class LlamaInference {
    constructor() {
        ...
        this.maxPrefixCacheSize = 100;
        this._generating = false;
        this.logger = new shared_1.Logger('LlamaInference');
        this.gpuDetector = new gpu_detector_1.GPUDetector();
        this.duplicationDetector = new language_1.DuplicationDetector({
            similarityThreshold: 0.85,
            minBlockSize: 20,
            detectDistributed: true
        });
        this.astParser = new language_2.ASTParser();
        this.fimManager = new fim_templates_1.FIMManager();
    }
    async getLlamaInstance() {
        if (!LlamaInference._llamaInstance) {
            try {
                ...
                this.logger.error('Failed to initialize node-llama-cpp:', error);
    ...
    async generateCompletion(prompt, options = {}, onToken, cancellationToken) {
        ...
        try {
            ...
            const lineFingerprints = new Map(); // Track line fingerprints
                    ...
                    if (allLines.length > 2) {
                        ...
                        if (lastCompleteLine.length > 5) {
                            ...
                            const exactMatches = Array.from(lineFingerprints.entries())
                                .filter(([lineNum, fp]) => lineNum < currentLineNumber &&
                                fp === fingerprint.md5)
                                .length;
                            ...
                            for (const [lineNum, fp] of lineFingerprints.entries()) {
                                ...
                                if (prevLine.length > 5) {
                                    const similarity = this.duplicationDetector.calculateSimilarity(lastCompleteLine, prevLine);
...
exports.LlamaInference = LlamaInference;
// Constants
LlamaInference.DEFAULT_MAX_TOKENS = 512;
LlamaInference.DEFAULT_TEMPERATURE = 0.7;
LlamaInference.DEFAULT_TOP_P = 0.95;
LlamaInference.DEFAULT_TOP_K = 40;
LlamaInference.DEFAULT_REPEAT_PENALTY = 1.2;
LlamaInference.DEFAULT_CONTEXT_SIZE = 16384;
/**
 ...
 */
LlamaInference._llamaInstance = null;
