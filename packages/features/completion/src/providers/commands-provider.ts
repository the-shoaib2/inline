...
import { DocGenerator } from '../generation/doc-generator';
import { TypeChecker } from '@inline/language';
import { PRGenerator } from '../generation/pr-generator';
import { ModelManager, ErrorExplainer, SecurityScanner, RefactoringEngine } from '@inline/intelligence';
...
`;
    ...
    private async handleSuggestCommand(): Promise<void> {
        vscode.window.showInformationMessage('Terminal Assistant is currently disabled.');
        return;
    ...
    private async handleExplainCommand(): Promise<void> {
        vscode.window.showInformationMessage('Command explanation is disabled.');
    }
