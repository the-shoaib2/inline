...
class ContextAnalyzer {
     ...
     */
    async analyzeRelatedFiles(currentFile, imports) {
        const relatedFiles = [];
        ...
        for (const importInfo of imports.slice(0, 5)) {
            try {
                const resolvedPath = await this.resolveImportPath(importInfo.module, currentFile, workspaceFolder);
                if (resolvedPath) {
                    ...
                    const relevantCode = this.extractRelevantCode(document, importInfo.imports, symbols);
                    relatedFiles.push({
     ...
     */
    async resolveImportPath(moduleName, currentFile, workspaceFolder) {
        ...
        for (const pattern of srcPatterns) {
            if (moduleName.startsWith(pattern)) {
                ...
                for (const ext of extensions) {
                    try {
                        const resolvedPath = path.join(workspaceFolder.uri.fsPath, 'src', relativePath + ext);
                        const uri = vscode.Uri.file(resolvedPath);
                        await vscode.workspace.fs.stat(uri);
                        return uri;
                    }
     ...
     */
    processSymbolsRecursive(symbols, result, document) {
     ...
     */
    extractRelevantCode(document, importedSymbols, allSymbols) {
        const relevantCode = [];
        ...
        for (const symbolName of importedSymbols) {
            ...
            if (symbol) {
                ...
                const endLine = Math.min(symbol.location.range.end.line, startLine + 20);
     ...
     */
    async buildProjectSymbolTable(workspaceFolder) {
        const projectSymbols = new Map();
        try {
            // Find all TypeScript/JavaScript files, excluding node_modules
            const files = await vscode.workspace.findFiles(new vscode.RelativePattern(workspaceFolder, '**/*.{ts,tsx,js,jsx}'), '**/node_modules/**', 100 // Limit to 100 files for performance
            );
     ...
     */
    async findSimilarCode(codeSnippet, language, workspaceFolder) {
        const similarBlocks = [];
        try {
            ...
            const files = await vscode.workspace.findFiles(new vscode.RelativePattern(workspaceFolder, `**/*.{${this.getExtensions(language)}}`), '**/node_modules/**', 50 // Limit search scope
            );
     ...
     */
    async detectCodingPatterns(files) {
     ...
     */
    detectPattern(text, regex, patternName, patterns) {
     ...
     */
    async buildDependencyGraph(currentFile, imports) {
